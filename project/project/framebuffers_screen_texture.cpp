...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!output) {
			...
			for (int row = 0; row < 4; row++) {
				for (int column = 0; column < 4; column++) {
					std::cout << transposed[row][column] << " ";
				}
				std::cout << std::endl;
			}
			output = true;
		...
		*/
		...
		glUniform1i(glGetUniformLocation(screenShader.Program, "grayscaleChoice"), grayscaleChoice);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		glBindVertexArray(0);
...
GLuint generateAttachmentTexture(GLboolean depth, GLboolean stencil)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); //so no texture wrapping when texture grab goes outside range
